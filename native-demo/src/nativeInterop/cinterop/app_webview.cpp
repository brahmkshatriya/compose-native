...
static SDL_SystemCursor app_webview_cursor_type(const char *name) {
    if (!std::strcmp(name, "pointer")) return SDL_SYSTEM_CURSOR_HAND;
    ...
    if (!std::strcmp(name, "progress")) return SDL_SYSTEM_CURSOR_WAITARROW;
...
static void app_webview_set_cursor_from_name(WPEView *wpe_view, const char *name) {
    ...
    if (view->custom_cursor) {
        SDL_FreeCursor(view->custom_cursor);
...
static void app_webview_set_cursor_from_bytes(
    ...
    if (!cursor) return;
    if (view->custom_cursor) SDL_FreeCursor(view->custom_cursor);
...
void app_webview_destroy(AppWebView *view) {
    ...
    if (view->custom_cursor) {
        SDL_SetCursor(app_webview_system_cursor(SDL_SYSTEM_CURSOR_ARROW));
        SDL_FreeCursor(view->custom_cursor);
    }
    app_webview_release_buffer(view);
