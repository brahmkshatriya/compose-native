...
internal actual fun String.findPrecedingBreak(index: Int): Int {
    return offsetByCodePoints(index, -1)
}
...
internal fun CharSequence.offsetByCodePoints(index: Int, offset: Int): Int {
    ...
    for (i in 0 until distance) {
        currentOffset += sign
        if (currentOffset <= 0) return 0 else if (currentOffset >= length) return length
...
// Copy from
// https://github.com/JetBrains/kotlin/blob/7cd306950aad852e006715067435a4bbd9cd40d2/kotlin-native/runtime/src/main/kotlin/generated/_StringUppercase.kt#L26
...
/**
 * Finds the offset of the next non-whitespace symbols subsequence (word) in the given text starting
 * from the specified caret offset.
 *
 ...
 * @return The offset of the next non-whitespace symbols subsequence (word), or the end of the
...
internal fun findNextNonWhitespaceSymbolsSubsequenceStartOffset(
     ...
     */
    var currentOffset = offset.coerceIn(0, currentText.length)
    while (currentOffset < currentText.length && !currentText[currentOffset].isWhitespace()) {
        currentOffset = currentText.offsetByCodePoints(currentOffset, 1)
    }
    while (currentOffset < currentText.length && currentText[currentOffset].isWhitespace()) {
        currentOffset = currentText.offsetByCodePoints(currentOffset, 1)
    }
...
/**
 * Checks if the character at the specified offset in the given string is either a whitespace or
 ...
 * symbols. It counts the number of symbols in the string to determine the midpoint position.
 *
 * @return The midpoint position in the string.
 */
internal fun String.midpointPositionWithUnicodeSymbols(): Int {
    return offsetByCodePoints(0, codePointCount() / 2)
}
