...
namespace Clipper2Lib {
  ...
  inline bool Path2ContainsPath1(const Path<T>& path1, const Path<T>& path2)
  {
    ...
    {
      PointInPolygonResult current = PointInPolygon(pt, path2);
      if (current == PointInPolygonResult::IsOutside)
      {
        if (pip == PointInPolygonResult::IsOutside) return false;
        pip = current;
      }
      else if (current == PointInPolygonResult::IsInside)
      {
        if (pip == PointInPolygonResult::IsInside) return true;
        pip = current;
      }
    }
    return pip == PointInPolygonResult::IsInside;
  }
