...
internal actual fun configureNativeSdlEnvironment() {
...
internal actual object NativeDesktopIntegration {
    actual fun install() {
        NativeDesktopPlatformServicesRegistry.install(LinuxSdlPlatformServices)
    }
    ...
    actual fun pollAccessibility(): Boolean = kld_atspi_poll() != 0

    actual fun updateSystemTheme(dark: Boolean?) {
...
private object LinuxSdlPlatformServices : NativeDesktopPlatformServices {
    ...
    override fun updateProgressJob(path: String, update: NativeProgressUpdate) = memScoped {
        val errorPointer = alloc<CPointerVar<ByteVar>>()
        errorPointer.value = null
        val updated =
            kld_progress_update(
    ...
    override fun pollDesktopEvent(): NativeDesktopEvent? = memScoped {
        ...
        when (type) {
            ...
            3 ->
                NativeDesktopEvent.ProgressRequested(
                    text,
