...
import sdl2.SDL_DelayNS
import sdl2.SDL_GetClipboardText
import sdl2.SDL_GetError
import sdl2.SDL_GetPointerProperty
import sdl2.SDL_GetWindowProperties
import sdl2.SDL_OpenURL
import sdl2.SDL_PROP_WINDOW_WIN32_HWND_POINTER
...
internal fun nativeGetEnvironmentVariable(name: String): String? = SDL_getenv(name)?.toKString()

internal fun nativeSleepMicroseconds(value: UInt) {
    SDL_DelayNS(value.toULong() * 1_000uL)
}

internal fun configureNativeSdlEnvironment() = Unit
...
internal object NativeDesktopIntegration {
    fun install() {
        LinuxPlatformServicesRegistry.install(WindowsSdlPlatformServices)
    }
    ...
    fun pollAccessibility(): Boolean = false

    fun updateSystemTheme(dark: Boolean?) = Unit
}

private object WindowsSdlPlatformServices : LinuxPlatformServices {
    ...
    override fun updateProgressJob(path: String, update: LinuxProgressUpdate) {
