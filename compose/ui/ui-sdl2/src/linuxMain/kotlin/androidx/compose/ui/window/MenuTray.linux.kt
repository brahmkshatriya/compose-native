...
internal sealed interface LinuxMenuEntry {
    val id: Int
    val enabled: Boolean
    ...
    data class Menu(
        ...
        val mnemonic: Char?,
        val children: List<LinuxMenuEntry>,
    ) : LinuxMenuEntry
    ...
    data class Separator(override val id: Int) : LinuxMenuEntry {
        override val enabled: Boolean = false
    }
}

internal data class LinuxMenuModel(val entries: List<LinuxMenuEntry>) {
    ...
    companion object {
        val Empty = LinuxMenuModel(emptyList())
    }
}

private fun LinuxMenuEntry.presentationHash(): Int =
    when (this) {
        is LinuxMenuEntry.Separator -> 17
...
private fun LinuxMenuEntry.findShortcut(event: KeyEvent): LinuxMenuEntry.Item? =
    when (this) {
        is LinuxMenuEntry.Menu -> children.firstNotNullOfOrNull { it.findShortcut(event) }
        is LinuxMenuEntry.Item ->
            takeIf { item ->
        ...
        is LinuxMenuEntry.Separator -> null
    }

internal class LinuxMenuBuilder(private val ids: MenuIdAllocator = MenuIdAllocator()) {
    val entries = mutableListOf<LinuxMenuEntry>()

    fun nextId(): Int = ids.next()

    fun child(): LinuxMenuBuilder = LinuxMenuBuilder(ids)

    fun build(): LinuxMenuModel = LinuxMenuModel(entries.toList())
}
...
class MenuBarScope internal constructor(private val builder: LinuxMenuBuilder) {
    @Composable
    @MenuComposable
    fun Menu(
        text: String,
        ...
        builder.entries +=
            LinuxMenuEntry.Menu(builder.nextId(), text, enabled, mnemonic, children.entries.toList())
    }
}

class MenuScope internal constructor(private val builder: LinuxMenuBuilder) {
    @Composable
    @MenuComposable
    fun Menu(
        text: String,
    ...
    @MenuComposable
    fun Separator() {
        builder.entries += LinuxMenuEntry.Separator(builder.nextId())
    }
...
fun FrameWindowScope.MenuBar(
    ...
    DisposableEffect(window.host) {
        onDispose { window.host.updateMenuBar(LinuxMenuModel.Empty) }
    }
}
...
@Composable
internal fun LinuxWindowMenuBar(model: LinuxMenuModel) {
    ...
    Row(
        ...
        model.entries.filterIsInstance<LinuxMenuEntry.Menu>().forEach { menu ->
            Box {
                BasicText(
                    text = menu.text,
                    modifier =
                ...
                if (openMenuId == menu.id) {
                    Popup(
                        ...
                        LinuxMenuPopup(menu.children) { openMenuId = 0 }
                    }
                }
            }
        }
...
@Composable
private fun LinuxMenuPopup(entries: List<LinuxMenuEntry>, closeAll: () -> Unit) {
    Column(
        Modifier.widthIn(min = 190.dp).background(MenuPopupBackground).padding(vertical = 4.dp)
    ) {
        entries.forEach { entry -> LinuxMenuEntryView(entry, closeAll) }
    }
}

@Composable
private fun LinuxMenuEntryView(entry: LinuxMenuEntry, closeAll: () -> Unit) {
            ...
            Box {
                ...
                if (expanded) {
                    Popup(
                        ...
                        LinuxMenuPopup(entry.children, closeAll)
                    }
                }
            }
        }
