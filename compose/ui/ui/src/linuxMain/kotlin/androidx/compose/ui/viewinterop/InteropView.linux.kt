 ...
 *
 * The pointer is valid only for the duration of [LinuxInteropView.render]. Native integrations
...
enum class InteropPointerEventType { Move, Button, Scroll }
 ...
 *
 * Rendering into a Compose-owned framebuffer works consistently on Wayland and X11, unlike
 * foreign child-window embedding, which is not a portable Wayland facility.
 */
class LinuxInteropView private constructor(
    ...
    fun sendPointerEvent(event: InteropPointerEvent): Boolean = pointerHandler?.invoke(event) == true
    ...
    /**
     * Installs the callback used by the platform host to invalidate the Compose draw node.
     * A [LinuxInteropView] is owned by one [NativeView] at a time.
     */
