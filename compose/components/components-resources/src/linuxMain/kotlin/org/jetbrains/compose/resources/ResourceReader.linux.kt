...
internal actual fun getPlatformResourceReader(): ResourceReader = LinuxResourceReader

@ExperimentalResourceApi
private object LinuxResourceReader : ResourceReader {
...
private fun findResourceRoot(): String {
    ...
    val executableDirectory = executablePath()?.substringBeforeLast('/', "")
...
private fun executablePath(): String? = memScoped {
    ...
    val length = readlink("/proc/self/exe", buffer, 4095.convert())
    if (length <= 0) null
    else {
...
private fun readFile(path: String, offset: Long, requestedSize: Long?): ByteArray {
    ...
    try {
        ...
        if (fseek(file, safeOffset, SEEK_SET) != 0)
