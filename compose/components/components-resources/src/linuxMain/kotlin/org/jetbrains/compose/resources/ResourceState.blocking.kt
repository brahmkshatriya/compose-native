...
internal actual fun <T> rememberResourceState(
    ...
    return remember(key1, environment) { mutableStateOf(runBlocking { block(environment) }) }
}
...
internal actual fun <T> rememberResourceState(
    ...
    val environment = LocalComposeEnvironment.current.rememberEnvironment()
    return remember(key1, key2, environment) { mutableStateOf(runBlocking { block(environment) }) }
}
...
internal actual fun <T> rememberResourceState(
    ...
    return remember(key1, key2, key3, environment) {
        mutableStateOf(runBlocking { block(environment) })
    }
}
