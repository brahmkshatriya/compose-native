...
@InternalResourceApi
constructor(id: String, val key: String, items: Set<ResourceItem>) : Resource(id, items)

/**
 * Retrieves the string for the pluralization for the given quantity using the specified quantity
 * string resource.
 *
 ...
 * @throws IllegalArgumentException If the provided ID or the pluralization is not found in the
 *   resource file.
 */
@Composable
fun pluralStringResource(resource: PluralStringResource, quantity: Int): String {
    ...
    val pluralStr by
        rememberResourceState(resource, quantity, { "" }) { env ->
...
private suspend fun loadPluralString(
    ...
    val str =
        item.items[pluralCategory]
            ...
            ?: error(
                "Quantity string ID=`${resource.key}` does not have the pluralization $pluralCategory for quantity $quantity!"
            )
    return str
}
...
fun pluralStringResource(
    ...
    val pluralStr by
        rememberResourceState(resource, quantity, args, { "" }) { env ->
