 ...
 */
@Immutable
class DrawableResource @InternalResourceApi constructor(id: String, items: Set<ResourceItem>) :
    Resource(id, items)

/**
 * Retrieves a [Painter] using the specified drawable resource. Automatically select a type of the
 * Painter depending on the file extension.
 *
...
fun imageResource(resource: DrawableResource): ImageBitmap {
    ...
    val imageBitmap by
        rememberResourceState(
            ...
            val resourceDensityQualifier =
                item.qualifiers.firstOrNull { it is DensityQualifier } as? DensityQualifier
            ...
            val path = item.path
            val cached =
                loadImage(path, "$path-${screenDensity}dpi", resourceReader) {
...
fun vectorResource(resource: DrawableResource): ImageVector {
    ...
    val imageVector by
        rememberResourceState(resource, resourceReader, density, { emptyImageVector }) { env ->
            val path = resource.getResourceItemByEnvironment(env).path
            val cached =
                loadImage(path, path, resourceReader) {
...
private fun svgPainter(resource: DrawableResource): Painter {
    ...
    val density = LocalDensity.current
    val svgPainter by
        rememberResourceState(resource, resourceReader, density, { emptySvgPainter }) { env ->
            val path = resource.getResourceItemByEnvironment(env).path
            val cached =
                loadImage(path, path, resourceReader) {
 ...
 *
 * @param environment The resource environment, which can be obtained from
 *   [rememberResourceEnvironment] or [getSystemResourceEnvironment].
