...
internal interface ComposeEnvironment {
    @Composable fun rememberEnvironment(): ResourceEnvironment
}

internal val DefaultComposeEnvironment =
...
/**
 * Provides the resource environment for non-composable access to resources. It is an expensive
 * operation! Don't use it in composable functions with no cache!
 */
    ...
    items
        ...
        .let { items ->
            if (items.isEmpty()) {
                ...
                error(
                    "Resource with ID='$id' has more than one file: ${items.joinToString { it.path }}"
                )
            }
        }
}
...
private fun List<ResourceItem>.filterBy(qualifier: Qualifier): List<ResourceItem> {
    ...
    // but it provides the same result:
    ...
    val withQualifier = filter { item -> item.qualifiers.any { it == qualifier } }

    if (withQualifier.isNotEmpty()) return withQualifier

    // items with no requested qualifier type (default)
    return filter { item -> item.qualifiers.none { it::class == qualifier::class } }
}

// https://developer.android.com/guide/topics/resources/providing-resources#BestMatch
// In general, Android prefers scaling down a larger original image to scaling up a smaller original
// image.
private fun List<ResourceItem>.filterByDensity(density: DensityQualifier): List<ResourceItem> {
    ...
    val exactAndHigherQualifiers =
        DensityQualifier.entries.filter { it.dpi >= density.dpi }.sortedBy { it.dpi }
    ...
    // The system assumes that default resources (those from a directory without configuration
    // qualifiers)
    ...
    val withNoDensity = items.filter { item -> item.qualifiers.none { it is DensityQualifier } }
    if (withNoDensity.isNotEmpty()) return withNoDensity

    // items with LDPI density
    return items.filter { item -> item.qualifiers.any { it == DensityQualifier.LDPI } }
}
...
private fun List<ResourceItem>.filterByLocale(
    ...
    val withExactLocale = withLanguage.filter { item -> item.qualifiers.any { it == region } }

    // if there are the exact language + the region items
    if (withExactLocale.isNotEmpty()) return withExactLocale

    val withDefaultRegion =
        withLanguage.filter { item -> item.qualifiers.none { it is RegionQualifier } }
