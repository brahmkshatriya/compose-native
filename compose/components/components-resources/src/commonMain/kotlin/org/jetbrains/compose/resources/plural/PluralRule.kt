...
internal class PluralRule
private constructor(val category: PluralCategory, private val condition: Condition) {

    constructor(
        category: PluralCategory,
        condition: String,
    ) : this(category, Condition.parse(condition))
    ...
    private sealed class Condition {
        ...
        /**
         * Plural operands defined in the
         * [Unicode Locale Data Markup Language](https://unicode.org/reports/tr35/tr35-numbers.html#Plural_Operand_Meanings).
         */
        enum class Operand {
            ...
            /**
             * The visible fraction digits in the source number, *with* trailing zeros, expressed as
             * an integer.
             */
            F,

            /**
             * The visible fraction digits in the source number, *without* trailing zeros, expressed
             * as an integer.
             */
            T,

            /**
             * Compact decimal exponent value: exponent of the power of 10 used in compact decimal
             * formatting.
             */
            C,
        }
        ...
        class And(private val left: Condition, private val right: Condition) : Condition() {
            ...
            override fun equivalentForInteger(other: Condition): Boolean {
                ...
                return left.equivalentForInteger(other.left) &&
                    right.equivalentForInteger(other.right)
            }
        ...
        class Relation(
            ...
            override fun isFulfilled(n: Int): Boolean {
                val expressionOperandValue =
                    when (operand) {
                        Operand.N,
                        Operand.I -> n.absoluteValue
                        else -> 0
                    }
                val moduloAppliedValue =
            ...
            override fun simplifyForInteger(): Condition {
                return when (operand) {
                    Operand.N,
                    Operand.I -> Relation(Operand.N, operandDivisor, comparisonIsNegated, ranges)
            ...
            override fun equivalentForInteger(other: Condition): Boolean {
                ...
                if (
                    (operand == Operand.N || operand == Operand.I) !=
                        (other.operand == Operand.N || other.operand == Operand.I)
                )
                    return false
        ...
        private class Parser(private val description: String) {
            ...
            private fun raise(): Nothing =
                throw PluralRuleParseException(description, currentIdx + 1)
            ...
            private fun consumeNextInt(): Int {
                ...
                while (
                    integerLastIdx < description.length && description[integerLastIdx].isDigit()
                ) {
                    integerValue *= 10
        ...
        companion object {
            /**
             * Parses [description] as defined in the
             * [Unicode Plural rules syntax](https://unicode.org/reports/tr35/tr35-numbers.html#Plural_rules_syntax).
             * For compact implementation, samples and keywords for backward compatibility are also
             * not handled. You can find such keywords in the
             * [Relations Examples](https://unicode.org/reports/tr35/tr35-numbers.html#Relations_Examples)
             * section.
             *
             * ```
             * condition       = and_condition ('or' and_condition)*
             ...
             * ```
             */
            fun parse(description: String): Condition =
                Parser(description).parse().simplifyForInteger()
        }
    }
}
